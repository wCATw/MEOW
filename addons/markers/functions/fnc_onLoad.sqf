...
_channButt = _display displayCtrl IDC_BUTTON_CHANNEL;
...
} forEach [IDC_CONTROLS_GROUP_ADV, IDC_CONTROLS_GROUP];
{
	(_display displayCtrl _x) ctrlShow false;
...
_picture ctrlSetPosition [
	(GVAR(displayCoord) select 0) - ((ctrlPosition _picture) select 2)/2,
	(GVAR(displayCoord) select 1)-((ctrlPosition _picture) select 3)/2,
	(ctrlPosition _picture) select 2,
	(ctrlPosition _picture) select 3
];
_picture ctrlCommit 0;
_text ctrlSetPosition [
	(GVAR(displayCoord) select 0) - ((ctrlPosition _picture) select 2)/2 + 0.07,
	(GVAR(displayCoord) select 1)-((ctrlPosition _text) select 3)/2,
	(ctrlPosition _text) select 2,
	(ctrlPosition _text) select 3
];
(_text) ctrlCommit 0;
...
_pos set [3,0];
_swt_info_group ctrlSetPosition [
	_pos select 0,
	_pos select 1,
	((ctrlPosition _swt_info_group) select 2)+0.69 * (((safeZoneW / safeZoneH) min 1.2) / 40),
	0
];
...
if (GVAR(saveText)) then {
	_text ctrlSetText GVAR(text);
};
if !(GVAR(showButt)) then {
	// Hide OK/Cancel if not needed
	{
		(_display displayCtrl _x) ctrlSetPosition [
			(ctrlPosition (_display displayCtrl _x)) select 0,
			(ctrlPosition (_display displayCtrl _x)) select 1,
			(ctrlPosition (_display displayCtrl _x)) select 2,
			0
		];
		(_display displayCtrl _x) ctrlCommit 0;
	} forEach [1,2];
...
// Arrange color controls, set color and visibility
{
	...
	_control ctrlSetPosition [
		((_pos_control_base select 0) + (((_pos_control_base select 2)/6) * _forEachIndex)),
		(_pos_control_base select 1) - (_pos select 3),
		_pos select 2,
		_pos select 3
	];
...
// Arrange icon picture controls, set icon and color, hide if needed
{
	...
	_control ctrlSetPosition [
		((_pos_control_base select 0) + (((_pos_control_base select 2)/6) * _forEachIndex)),
		(_pos_control_base select 1) - (_pos select 3) - 0.2*(_pos_control_base select 3) - (_coef_h_icon * ((_pos_control_base select 2)/6)),
		((_pos_control_base select 2)/6),
		(_coef_h_icon * ((_pos_control_base select 2)/6))
	];
	if (!isNil {GVAR(colorArr)}) then {
...
// Arrange fast text buttons
{
	...
	_control ctrlSetPosition [
		(_pos_control_base select 0) + (_pos_control_base select 2) - (_control_pos select 2)*(_forEachIndex+1),
		(_pos_control_base select 1),
		_control_pos select 2,
		_control_pos select 3
	];
	_control ctrlCommit 0;
...
_control_pic_pos = ctrlPosition _picture;
{
	...
	_control_pos = ctrlPosition _control;
	_control ctrlSetPosition [
		(_control_pic_pos select 0) - ((3*(((safeZoneW / safeZoneH) min 1.2) / 40))*(_forEachIndex+1)),
		(_control_pic_pos select 1) + (_control_pic_pos select 3)/2 - (_control_pos select 3)/2,
		(_control_pos select 2),
		(_control_pos select 3)
	];
	_control ctrlCommit 0;
...
};
{
	...
	_control ctrlSetPosition [
		(_pos_control_button select 0) + (_pos_control_button select 2)/2 + (_pos_control_button select 2)*0.02,
		(_pos_control_button select 1) + (_pos_control_button select 3)*1 + (0.3 * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25)) + (_pos_control select 3)*_forEachIndex + _butt_cor_pos,
		_pos_control select 2,
		_pos_control select 3
	];
	_control ctrlCommit 0;
} forEach _combo_icon;

{
	...
	_pos_control = (ctrlPosition _control);
	_control ctrlSetPosition [
		(_pos_control_button select 0) + (_pos_control_button select 2)/2 - (_pos_control_button select 2)*0.02 - (_pos_control select 2),
		(_pos_control_button select 1) + (_pos_control_button select 3)*1+(0.3 * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25)) + (_pos_control select 3)*_forEachIndex + _butt_cor_pos,
		_pos_control select 2,
		_pos_control select 3
	];
...
_pos = ctrlPosition _control;
_control ctrlSetPosition [
	(_pos_control_button select 0),
	(_pos_control_button select 1) + (_pos_control_button select 3)*1 + (0.6 * ((((safeZoneW / safeZoneH) min 1.2) / 1.2) / 25)) + ((ctrlPosition (_display displayCtrl IDC_COMBO_11)) select 3)*6 + _butt_cor_pos,
	_pos select 2,
	_pos select 3
];
_control ctrlCommit 0;
...
_control ctrlSetPosition [
	_pos select 0,
	(_pos select 1) + (_pos select 3)*1.3,
...
_pos = ctrlPosition _control;
_control ctrlSetPosition [
	(ctrlPosition (_text) select 0) + (ctrlPosition (_text) select 2),
	(ctrlPosition (_text) select 1),
	_pos select 2,
	_pos select 3
];
_control ctrlCommit 0;
