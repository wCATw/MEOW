...
switch (_action) do
{
	case "SHOW OK":
	{
		...
		{
			_pos set [1,_posY + 10 * _posH + 3 * 0.005];
		};
		...
		};
		{
			...
			_control ctrlSetPosition [
				_pos_control select 0,
				(_pos_control select 1) + _butt_cor_pos,
		...
		if !(GVAR(showButt)) then
		{
			...
			{
				private _ctrlToHide = _display displayCtrl _x;
				private _ctrlPos = ctrlPosition _ctrlToHide;
				_ctrlToHide ctrlSetPosition [
					_ctrlPos select 0, // X position remains unchanged
					_ctrlPos select 1, // Y position remains unchanged
					_ctrlPos select 2, // Width remains unchanged
					0                  // Height set to 0 to hide the control
				];
	...
	case "SHOW _INFO":
	{
		...
		(_display displayCtrl IDC_MENU_INFO) ctrlSetPosition [
			_pos select 0,
			_pos select 1,
			_pos select 2,
			if (GVAR(showInfo)) then
